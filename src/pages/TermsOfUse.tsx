          ...
          {sectionKeys.map((key, index) => (
            <section key={key}>
              <h2 className="text-xl font-bold text-castas-orange mb-2">
                {index + 1}. {t(`termsOfUse.sections.${key}.title`)}
              </h2>
              
              {/* Main content if exists */}
              {t(`termsOfUse.sections.${key}.content`) && (
                <p>{t(`termsOfUse.sections.${key}.content`)}</p>
              )}
              
              {/* Intro if exists */}
              {t(`termsOfUse.sections.${key}.intro`) && (
                <p>{t(`termsOfUse.sections.${key}.intro`)}</p>
              )}
              
              {/* Items if exist */}
              {(() => {
                const items = t(`termsOfUse.sections.${key}.items`, { returnObjects: true });
